...
public:
	Kub(){
		cube = (unsigned char**)malloc(sizeof(unsigned char*) * 6);
		for(int i = 0; i < 6; i++){
			cube[i] = (unsigned char*)malloc(sizeof(unsigned char) * 9);
			for(int j = 0; j < 9; j++){
				if(i == 0){
					cube[i][j] = 'y';
				}else if(i == 1){
	...
	void myspin(){
		cout << "Choose a color from the list (R, B, O, G, W, Y), only BIG characters and choose the direction of rotation (R, L)." << endl;
	...
	void getString(){
		str += ".";
		cout << str.length() / 3 << endl << str;
	}

	bool find(bool kek, int count){
		unsigned char** lol = (unsigned char**)malloc(sizeof(unsigned char*) * 6);
		for(int i = 0; i < 6; i++){
			lol[i] = (unsigned char*)malloc(sizeof(unsigned char) * 9);
			for(int j = 0; j < 9; j++){
				lol[i][j] = cube[i][j];
			}
		}
...
int main(int argc, char** argv){
	...
	do{
		...
		if(var == 1){
			...
			for(i = 0; i < 100000; i++){
				cout << i << endl;
				cube.read();//считаем что куб верный
				cube.scrol();
				cube.find(1, count);
				//cube.littlecheck(count);
			}
