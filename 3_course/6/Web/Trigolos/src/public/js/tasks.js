...
function change(allTasks, at) {
  for (let i = 0; i < allTasks.length / 2; i++) {
    ...
    let duo =
      at.children[allTasks.length - i - 1].children[1].children[0].checked;
...
function sortUp() {
  ...
  if (
    at.children[0].children[0].textContent.split('.')[0] <
    at.children[allTasks.length - 1].children[0].textContent.split('.')[0]
  ) {
    change(allTasks, at);
  }
}
...
function check(elem) {
  ...
  allTasks[textNum - 1 - count]['completed'] =
    allTasks[textNum - 1 - count]['completed'] !== true;
...
function deleteTask(elem) {
  let delTextNum =
    elem.parentNode.parentNode.children[0].textContent.split('.')[0];
  let allTasks = JSON.parse(locstor.getItem('tasks'));
